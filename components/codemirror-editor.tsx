...
export default function SimpleCodeEditor({
  ...
  const extractCodeSymbols = (doc: Text): CodeSymbol[] => {
    ...
    if (language === "javascript" || language === "typescript") {
      ...
      const varPattern =
        /(var|let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*(?:=|;|,)/g;
      let match;
      ...
      while ((match = classPattern.exec(text)) !== null) {
        ...
        if (classBodyStart !== -1) {
          ...
          while (
            (methodMatch = methodPattern.exec(methodSearchText)) !== null
          ) {
      ...
      while ((match = funcPattern.exec(text)) !== null) {
        ...
        while (lineIndex < lines.length) {
          ...
          if (line.trim() !== "" && line.search(/\S/) <= indentLevel) {
            funcBodyEnd =
              funcBodyStart + lines.slice(0, lineIndex).join("\n").length;
            break;
          }
          lineIndex++;
        }
      ...
      while ((match = classPattern.exec(text)) !== null) {
        ...
        while (lineIndex < lines.length) {
          ...
          if (line.trim() !== "" && line.search(/\S/) <= indentLevel) {
            classBodyEnd =
              classBodyStart + lines.slice(0, lineIndex).join("\n").length;
            break;
          }
          lineIndex++;
        }
      ...
      const varPattern =
        /\b(?:int|float|double|char|bool|string|auto|void|long|short)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g;
      let match;
      ...
      // Extract function declarations
      const funcPattern =
        /\b(?:int|float|double|char|bool|string|auto|void|long|short)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*$$([^)]*)$$/g;
      while ((match = funcPattern.exec(text)) !== null) {
        const name = match[1];
        const params = match[2];
        const from = match.index + match[0].indexOf(name);
        ...
        if (funcBodyStart !== -1) {
          ...
          const returnType = match[0]
            .substring(0, match[0].indexOf(name))
            .trim();
  ...
  const findScopeAtPosition = (pos: number): Scope | undefined => {
    ...
    const sortedScopes = [...scopesRef.current].sort(
      (a, b) => a.to - a.from - (b.to - b.from)
    );

    return sortedScopes.find((scope) => pos >= scope.from && pos <= scope.to);
  };
  ...
  const findDuplicateDeclarations = (symbols: CodeSymbol[]): Diagnostic[] => {
    ...
    symbolsByScope.forEach((scopeSymbols) => {
      scopeSymbols.forEach((symbolsWithSameName, name) => {
        if (symbolsWithSameName.length > 1) {
          ...
          const variableDuplicates = symbolsWithSameName.filter(
            (s) => s.type === "variable"
          );
          const functionDuplicates = symbolsWithSameName.filter(
            (s) => s.type === "function"
          );
            ...
            for (let i = 1; i < variableDuplicates.length; i++) {
              diagnostics.push({
                ...
                message: `Duplicate variable '${name}'. Previously declared at line ${
                  viewRef.current?.state.doc.lineAt(variableDuplicates[0].from)
                    .number
                }.`,
              });
            }
            ...
            for (let i = 1; i < functionDuplicates.length; i++) {
              diagnostics.push({
                ...
                message: `Duplicate function '${name}'. Previously declared at line ${
                  viewRef.current?.state.doc.lineAt(functionDuplicates[0].from)
                    .number
                }.`,
              });
            }
  ...
  const symbolCompletions = (
    ...
    if (isDotCompletion) {
      ...
      const objProperties = symbolsRef.current
        .filter(
          (s) => s.type === "property" && s.name.startsWith(`${objName}.`)
        )
        .map((s) => ({
